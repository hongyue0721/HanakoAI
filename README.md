...
- 支持历史记录查看
- 支持自定义模型提供方、模型和助手提示词
- 支持联网搜索（可选）：识别题目后由 LLM 判断是否需要搜索，并自动将搜索结果注入对话上下文
...
| 手势 | 普通模式 | 自动模式 |
|------|---------|---------|
| 单击 | 打开截图框选面板 | 截取整屏并直接发送给 AI |
| 双击 | 展开快捷菜单 | 展开快捷菜单（处理中时双击则取消当前任务） |
| 长按 | 展开快捷菜单 | 进入多图截图模式；已有多图时长按则一起发送 |
| 拖动 | 移动悬浮球位置 | 移动悬浮球位置 |
...
应用在识别题目后，可调用当前选中的文本模型判断题目是否需要联网搜索；若需要，则自动提取关键词、调用搜索引擎，并把搜索结果作为上下文注入到 LLM 的 system/user prompt 中。
...
1. 进入应用设置 → 联网搜索。
2. 打开“启用联网搜索”。
3. 选择搜索引擎并填写对应的 Base URL 与 API Key。
4. （可选）开启“自动模式也搜索”，让悬浮窗自动识别时同样触发搜索。

当前支持以下搜索引擎：

| 搜索引擎 | 注册 / 获取 API Key | 默认 Base URL |
|---|---|---|
| Tavily | https://tavily.com/ | `https://api.tavily.com` |
| Brave Search | https://api.search.brave.com/ | `https://api.search.brave.com` |
| Serper (Google) | https://serper.dev/ | `https://google.serper.dev` |
| Custom | 任意兼容 Tavily 请求格式的代理端点 | 手动填写 |

> 注意：联网搜索会消耗对应搜索引擎的 API 额度；多模态直接传图模式暂不会触发联网搜索。
